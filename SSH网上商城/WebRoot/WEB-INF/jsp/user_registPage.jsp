...
<script>/* js校验表单 */
      ...
      function oncheck(){
         var username=document.getElementById("username").value;
         var qqq=createXMLHttpRequest();
         qqq.onreadystatechange=function(){
             if(qqq.readyState==4&&qqq.status==200){
                   var text=document.getElementById("shownamespan");
                   text.innerHTML=qqq.responseText;
             }
           
         }
         qqq.open("GET","${pageContext.request.contextPath}/user_findByName.action?time="+new Date().getTime()+"&username="+username,true);
         qqq.send();
      }
      /* 获取XMLHttpRequest对象 */
        function createXMLHttpRequest(){
          ...
          if(window.ActiveXObject){
              rqe=new ActiveXObject("Microsoft.XMLHttp");
          }
          else if(window.XMLHttpRequest){
              rqe=new XMLHttpRequest();
          }
          return rqe;
       }
      
      function checkImg(){
         var checkimg=document.getElementById("captchaImage");
					...
					<form id="registerForm" action="${pageContext.request.contextPath}/user_regist.action"  method="post" novalidate="novalidate" onsubmit="checkform()">
						<table>
							<tbody><tr>
								...
								<td>
									<input type="text" id="username" name="username" class="text" maxlength="20" onblur="oncheck()">
									<span id="shownamespan"></span>
								</td>
							</tr>
							<tr>
								<th>
							...
							<tr>
								...
								<tr>
									...
									<td>
										<span class="fieldSet">
											<input type="text" id="checkimg" name="checkimg" class="text captcha" maxlength="4" autocomplete="off"><img id="captchaImage" class="captchaImage" src="${pageContext.request.contextPath}/checkimg.action" onclick="checkImg()" title="点击更换验证码">
										</span>
